...
with db() as c:
    ...
    if richtung == "kauf":
        ...
        # Cash nur bei Limit/Stop reservieren, nicht bei Market (wird sofort ausgeführt)
        if typ != "market":
...
# Market Orders sofort über Market Maker ausführen
if typ == "market":
    with db() as c:
        m = c.execute("SELECT * FROM markt WHERE coin=?", (coin,)).fetchone()
        if m:
            if richtung == "kauf":
                preis = m["ask"]
                kosten = round(menge * preis, 2)
                konto_row = c.execute("SELECT cash FROM konto WHERE user_id=?", (uid,)).fetchone()
                if not konto_row or konto_row["cash"] < kosten:
                    c.execute("UPDATE orders SET status='storniert' WHERE id=?", (order_id,))
                    return jsonify({"error": f"Nicht genug Cash. Benötigt: ${kosten:,.2f}"}), 400
                # Cash abziehen
                c.execute("UPDATE konto SET cash=cash-? WHERE user_id=?", (kosten, uid))
                # Coins ins Depot — Market Maker liefert immer
                c.execute("""
                    INSERT INTO depot (user_id, coin, menge, avg_kaufpreis)
                    VALUES (?, ?, ?, ?)
                    ON CONFLICT(user_id, coin) DO UPDATE SET
                ...
                # Spread-Gewinn tracking
                spread_gewinn = round(menge * preis * m["gap"], 2)
                c.execute("""
                    INSERT INTO transaktionen (user_id, typ, betrag, beschreibung)
                    VALUES (1, 'spread_gewinn', ?, ?)
                """, (spread_gewinn, f"Market Maker Spread: {menge} {coin} @ {preis}"))
                ...
                preis = m["bid"]
                erloes = round(menge * preis, 2)
                depot_row = c.execute("SELECT menge FROM depot WHERE user_id=? AND coin=?", (uid, coin)).fetchone()
                if not depot_row or depot_row["menge"] < menge:
                    c.execute("UPDATE orders SET status='storniert' WHERE id=?", (order_id,))
                    return jsonify({"error": f"Nicht genug {coin} im Depot"}), 400
                # Coins aus Depot entfernen — Market Maker kauft immer
                neuer_bestand = round(depot_row["menge"] - menge, 8)
                if neuer_bestand < 0.000001:
                    c.execute("DELETE FROM depot WHERE user_id=? AND coin=?", (uid, coin))
                else:
                    c.execute("UPDATE depot SET menge=? WHERE user_id=? AND coin=?", (neuer_bestand, uid, coin))
                # Cash gutschreiben
...
```
    ...
    if order["richtung"] == "kauf" and order["typ"] != "market":
        rest_menge = order["menge"] - order["menge_gefuellt"]
        preis = order["limit_preis"] or order["stop_preis"] or 0
        if preis > 0:
...
f”━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n”
f”*Dieser Beleg wurde automatisch erstellt.*”
)
...
# DISCORD BRIDGE BOT — integriert in app.py
...
BANK_CHANNEL_ID    = 1486833105884151919
COMMAND_CHANNEL_ID = 1486841457485680654
STAATSBOT_ID       = 0  # 0 = Test Modus, echte ID: 1420071218987532401
MAX_BETRAG         = 10_000_000
MIN_BETRAG         = 1
...
if len(self.minute_counts[key]) >= 10:
return False, “Rate-Limit: Zu viele Transaktionen”
if len(self.day_counts[key]) >= 500:
return False, “Tages-Limit erreicht”
...
with bridge_db() as c:
c.execute(“INSERT OR IGNORE INTO processed_messages (message_id, aktion, discord_id, betrag) VALUES (?,?,?,?)”,
(str(message_id), aktion, str(discord_id), betrag))

def bridge_einzahlen(discord_id, betrag, message_id, bot_id):
discord_id = str(discord_id)
if bereits_verarbeitet(message_id):
return False, “Bereits verarbeitet”
if betrag < MIN_BETRAG or betrag > MAX_BETRAG:
return False, f”Ungültiger Betrag (min ${MIN_BETRAG}, max ${MAX_BETRAG:,.0f})”
ok, fehler = rate_limiter.check(f”ein_{discord_id}”)
if not ok:
return False, fehler
with bridge_db() as c:
...
uid = user[“id”]
c.execute(“UPDATE konto SET cash=cash+?, eingezahlt=eingezahlt+? WHERE user_id=?”, (betrag, betrag, uid))
c.execute(“INSERT INTO transaktionen (user_id, typ, betrag, beschreibung) VALUES (?,‘einzahlung’,?,‘Staatsbot’)”, (uid, betrag))
markiere_verarbeitet(message_id, “EINZAHLUNG”, discord_id, betrag)
return True, None

def bridge_auszahlen(discord_id, betrag, message_id=None, bot_id=None):
discord_id = str(discord_id)
if message_id and bereits_verarbeitet(message_id):
return False, “Bereits verarbeitet”
if betrag < MIN_BETRAG or betrag > MAX_BETRAG:
return False, f”Ungültiger Betrag”
ok, fehler = rate_limiter.check(f”aus_{discord_id}”)
if not ok:
return False, fehler
with bridge_db() as c:
user = c.execute(“SELECT u.id, k.cash FROM users u LEFT JOIN konto k ON u.id=k.user_id WHERE u.discord_id=?”, (discord_id,)).fetchone()
if not user:
return False, “Discord ID nicht registriert”
if (user[“cash”] or 0) < betrag:
return False, f”Nicht genug Geld. Verfügbar: ${user[‘cash’]:,.0f}”
c.execute(“UPDATE konto SET cash=cash-?, ausgezahlt=ausgezahlt+? WHERE user_id=?”, (betrag, betrag, user[“id”]))
c.execute(“INSERT INTO transaktionen (user_id, typ, betrag, beschreibung) VALUES (?,‘auszahlung’,?,‘Staatsbot’)”, (user[“id”], betrag))
if message_id:
markiere_verarbeitet(message_id, “AUSZAHLUNG”, discord_id, betrag)
return True, None

def get_bridge_user(discord_id):
with bridge_db() as c:
return c.execute(“SELECT u.*, k.cash, k.eingezahlt, k.ausgezahlt FROM users u LEFT JOIN konto k ON u.id=k.user_id WHERE u.discord_id=?”, (str(discord_id),)).fetchone()

def start_discord_bot():
“”“Startet den Discord Bridge Bot in einem separaten Thread”””
try:
from config import BRIDGE_TOKEN
except ImportError:
print(“⚠️  Kein BRIDGE_TOKEN in config.py — Discord Bot deaktiviert”)
return
...
async def on_message(message):
    ...
    if not message.author.bot:
        await bot.process_commands(message)
        return
    if STAATSBOT_ID != 0 and message.author.id != STAATSBOT_ID:
        log.warning(f"Unbekannter Bot ignoriert: {message.author.id}")
        return

    text = message.content.strip()
    channel = message.channel

    # Format: "Spieler 123 hat 1000$ eingezahlt"
    import re
    match = re.search(r'[Ss]pieler\s+(\d+)\s+hat\s+([\d.,]+)\$?\s+eingezahlt', text)
    if match:
        discord_id = match.group(1)
        try:
            betrag = float(match.group(2).replace(".", "").replace(",", ""))
        except:
            return
        await message.add_reaction("⏳")
        ok, fehler = bridge_einzahlen(discord_id, betrag, message.id, message.author.id)
        await message.remove_reaction("⏳", bot.user)
        if ok:
            user = get_bridge_user(discord_id)
            name = user["username"] if user else discord_id
            await message.add_reaction("✅")
            await channel.send(f"✅ **CRYPTEX — Zahlung eingegangen!**\n👤 `{name}` | 💵 **${betrag:,.0f}** | 💰 Guthaben: **${user['cash']:,.0f}**\n🌐 cryptex-gta.duckdns.org")
        else:
            await message.add_reaction("❌")
            await channel.send(f"❌ **CRYPTEX — Fehler!**\n{fehler}")
        return

    # Format: "EINZAHLUNG 123 1000"
    match2 = re.match(r'EINZAHLUNG\s+(\d+)\s+([\d.,]+)', text)
    if match2:
        discord_id = match2.group(1)
        try:
            betrag = float(match2.group(2).replace(",", "."))
        except:
            return
        await message.add_reaction("⏳")
        ok, fehler = bridge_einzahlen(discord_id, betrag, message.id, message.author.id)
        ...
        if ok:
            user = get_bridge_user(discord_id)
            await message.add_reaction("✅")
            await channel.send(f"✅ **CRYPTEX** | `{discord_id}` → **${betrag:,.0f}** | Guthaben: **${user['cash']:,.0f}**")
        else:
            await message.add_reaction("❌")
            await channel.send(f"❌ **CRYPTEX Fehler**: {fehler}")
        return

    # Format: "AUSZAHLUNG 123 1000"
    match3 = re.match(r'AUSZAHLUNG\s+(\d+)\s+([\d.,]+)', text)
    if match3:
        discord_id = match3.group(1)
        try:
            betrag = float(match3.group(2).replace(",", "."))
        except:
            return
        await message.add_reaction("⏳")
        ok, fehler = bridge_auszahlen(discord_id, betrag, message.id, message.author.id)
        await message.remove_reaction("⏳", bot.user)
        if ok:
            user = get_bridge_user(discord_id)
            await message.add_reaction("✅")
            await channel.send(f"✅ **CRYPTEX AUSZAHLUNG**\n`{discord_id}` | **${betrag:,.0f}** ausgezahlt\n💸 STAATSBOT_AUSZAHLUNG {discord_id} {betrag:.0f}")
        else:
            await message.add_reaction("❌")
            await channel.send(f"❌ **CRYPTEX Fehler**: {fehler}\n💸 STAATSBOT_RUECKBUCHUNG {discord_id} {betrag:.0f}")
        return

    await bot.process_commands(message)

@bot.tree.command(name="auszahlen", description="Geld von Cryptex auszahlen")
async def slash_auszahlen(interaction: discord_lib.Interaction, betrag: int):
    if interaction.channel_id != COMMAND_CHANNEL_ID:
        ch = interaction.guild.get_channel(COMMAND_CHANNEL_ID)
        await interaction.response.send_message(f"❌ Nur in {ch.mention if ch else f'<#{COMMAND_CHANNEL_ID}>'} nutzbar!", ephemeral=True)
        return
    await interaction.response.defer(ephemeral=True)
    discord_id = str(interaction.user.id)
    user = get_bridge_user(discord_id)
    if not user:
        await interaction.followup.send("❌ Kein Cryptex-Konto. Registriere dich auf: http://cryptex-gta.duckdns.org", ephemeral=True)
        return
    ok, fehler = bridge_auszahlen(discord_id, betrag)
    if ok:
        user_neu = get_bridge_user(discord_id)
        channel = bot.get_channel(BANK_CHANNEL_ID)
        if channel:
            await channel.send(f"💸 STAATSBOT_AUSZAHLUNG {discord_id} {betrag}")
        await interaction.followup.send(f"✅ **${betrag:,.0f}** ausgezahlt!\nVerbleibendes Guthaben: **${user_neu['cash']:,.0f}**", ephemeral=True)
    else:
        await interaction.followup.send(f"❌ {fehler}", ephemeral=True)

@bot.tree.command(name="konto", description="Cryptex Kontostand")
async def slash_konto(interaction: discord_lib.Interaction):
    if interaction.channel_id != COMMAND_CHANNEL_ID:
        ch = interaction.guild.get_channel(COMMAND_CHANNEL_ID)
        await interaction.response.send_message(f"❌ Nur in {ch.mention if ch else f'<#{COMMAND_CHANNEL_ID}>'} nutzbar!", ephemeral=True)
        return
    user = get_bridge_user(str(interaction.user.id))
    if not user:
        await interaction.response.send_message("❌ Kein Cryptex-Konto.\n🌐 http://cryptex-gta.duckdns.org", ephemeral=True)
        return
...
loop.run_until_complete(bot.start(BRIDGE_TOKEN))
```
...
t3.start()
# Discord Bridge Bot
t4 = threading.Thread(target=start_discord_bot, daemon=True)
t4.start()
