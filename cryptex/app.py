...
DISCORD_CLIENT_SECRET = os.environ.get("DISCORD_SECRET", "")  # wird per Umgebungsvariable gesetzt
...
STEUER_SATZ   = 0.25   # 25% Kapitalertragsteuer
    ...
    with db() as c:
        ...
        for ko in kauf_orders:
            for vo in verk_orders:
                ...
                steuer = max(0, gewinn * STEUER_SATZ)
                netto = kosten - steuer

                c.execute("UPDATE depot SET menge=menge-? WHERE user_id=? AND coin=?",
                          (handelsmenge, vo["user_id"], coin))
                c.execute("UPDATE konto SET cash=cash+?, steuern_gezahlt=steuern_gezahlt+? WHERE user_id=?",
                          (netto, steuer, vo["user_id"]))
...
def place_order():
    ...
    with db() as c:
        ...
        order_id = c.execute("""
            INSERT INTO orders (user_id, coin, typ, richtung, menge, limit_preis, status)
            VALUES (?, ?, ?, ?, ?, ?, 'offen')
        """, (uid, coin, typ, richtung, menge, limit_preis)).lastrowid
