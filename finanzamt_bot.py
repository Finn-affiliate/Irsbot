...
# Speichert welcher Discord-User auf welchen Akten-Kanal wartet
...
if kanal.name == kname:
return kanal, False
neuer = await kategorie.create_text_channel(
name=kname,
topic=“Aktenmappe fuer “ + person
)
return neuer, True
...
if neu:
    status = "Neue Akte angelegt"
else:
    status = "Neuer Antrag in bestehender Akte"
...
if discord_username:
    ...
    if user:
        ...
        try:
            await user.send(
                "Hallo " + person + ",\n\n"
                "dein Steuerantrag ist eingegangen.\n"
                "Bitte ueberweis den faelligen Betrag an die Staatsbank und "
                "schicke mir anschliessend ein Foto der Ueberweisung als Antwort auf diese Nachricht.\n\n"
                "Vielen Dank\nDas Finanzamt"
            )
        except discord.Forbidden:
            await kanal.send("Hinweis: Konnte keine DM an " + discord_username + " senden. DMs moeglicherweise deaktiviert.")
    else:
        await kanal.send(
            "Warnung: Discord-User '" + discord_username + "' wurde auf dem Server nicht gefunden. "
            "DM konnte nicht gesendet werden."
        )
else:
    await kanal.send("Hinweis: Kein Discord-Username im Antrag gefunden. DM wurde nicht gesendet.")
...
# DM vom User mit Ueberweisungsfoto
...
await message.author.send(“Danke! Dein Ueberweisungsbeleg wurde eingereicht und wird geprueft.”)
else:
...
# Pruefe ob ein Sachbearbeiter das Haekchen auf ein Ueberweisungsfoto gesetzt hat
...
# Pruefen ob es ein Ueberweisungsfoto ist (hat Attachments und wurde vom Bot gesendet)
if message.author != client.user or not message.attachments:
    return

# User finden dem die Bestaetigung geschickt werden soll
