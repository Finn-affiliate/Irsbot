...
async def on_message(message):
    ...
    if match_ein:
        ...
        if ok:
            ...
            await channel.send(
                f"✅ **CRYPTEX — Zahlung eingegangen!**\n"
                f"👤 Spieler: `{name}`\n"
                f"💵 Eingezahlt: **${betrag:,.0f}**\n"
                f"💰 Neues Guthaben: **${user['cash']:,.0f}**\n"
            ...
            await message.add_reaction("❌")
            await channel.send(
                f"❌ **CRYPTEX — Zahlung fehlgeschlagen!**\n"
    ...
    if match_aus:
        ...
        # ── CRYPTEX_OK vom Staatsbot ──
        match_ok = re.match(r'CRYPTEX_OK\s+(\d+)\s+([\d.,]+)', text)
        if match_ok:
            discord_id = match_ok.group(1)
            try:
                betrag = float(match_ok.group(2).replace(",", "."))
            except:
                return
            ok, fehler = einzahlen(discord_id, betrag, message.id, message.author.id)
            if ok:
                user = get_user_by_discord_id(discord_id)
                name = user["username"] if user else discord_id
                await message.add_reaction("✅")
                await channel.send(
                    f"✅ **CRYPTEX — Einzahlung bestätigt!**\n"
                    f"👤 `{name}` | 💵 **${betrag:,.0f}** | Guthaben: **${user['cash']:,.0f}**"
                )
            else:
                await message.add_reaction("❌")
        ...
        # ── CRYPTEX_ERROR vom Staatsbot ──
        match_err = re.match(r'CRYPTEX_ERROR\s+(\d+)\s+([\d.,]+)\s+(\S+)', text)
        if match_err:
            discord_id = match_err.group(1)
            try:
                betrag = float(match_err.group(2).replace(",", "."))
            except:
                return
            grund = match_err.group(3)
            await message.add_reaction("❌")
            if grund == "insufficient_funds":
                await channel.send(f"❌ **CRYPTEX** — `{discord_id}` hat nicht genug Geld. ${betrag:,.0f} wurde nicht abgezogen.")
            elif grund == "user_not_found":
                await channel.send(f"❌ **CRYPTEX** — Spieler `{discord_id}` nicht gefunden.")
            else:
                await channel.send(f"❌ **CRYPTEX Fehler** für `{discord_id}`: {grund}")
            return
...
@bot.tree.command(name="einzahlen", description="Geld von GTA ins Cryptex einzahlen")
async def slash_einzahlen(interaction: discord_lib.Interaction if False else discord.Interaction, betrag: int):
    if not await check_channel(interaction): return
    await interaction.response.defer(ephemeral=True)
    ...
    # Nachricht in Bank Channel schreiben damit Staatsbot es verarbeitet
    channel = bot.get_channel(BANK_CHANNEL_ID)
    if channel:
        await channel.send(f"Spieler {discord_id} hat {betrag}$ eingezahlt")

    await interaction.followup.send(
        f"⏳ Einzahlungsanfrage für **${betrag:,.0f}** wurde gesendet.\n"
        f"Der Staatsbot verarbeitet deine Anfrage...",
        ephemeral=True
    )


@bot.tree.command(name="einzahlen", description="Geld von GTA ins Cryptex einzahlen")
async def slash_einzahlen(interaction: discord.Interaction, betrag: int):
    if not await check_channel(interaction): return
    await interaction.response.defer(ephemeral=True)
    discord_id = str(interaction.user.id)
    user = get_user_by_discord_id(discord_id)
    ...
    if channel:
        await channel.send(f"Spieler {discord_id} hat {betrag}$ eingezahlt")
    await interaction.followup.send(
        f"⏳ Einzahlungsanfrage für **${betrag:,.0f}** gesendet.\n"
        f"Warte auf Bestätigung vom Staatsbot...",
        ephemeral=True
    )
